...
//			this.damage2 = damage2;
		...
		public void run() {
			...
			if(object.isHeal()) {
				...
				if(!object.isMP()) {
					damageDisplayEvent dde = new damageDisplayEvent(combating, damageDisplayEvent.DisplayType.PLAYER_HEAL, object.getHealAmount());						
					Thread t = new Thread(dde);						
					t.start();
				}else {
					damageDisplayEvent dde = new damageDisplayEvent(combating, damageDisplayEvent.DisplayType.PLAYER_MP_HEAL, object.getHealAmount());						
					Thread t = new Thread(dde);						
					t.start();
				}
				object.Heal();
